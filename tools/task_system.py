...
class TaskStore:
    ...
    def _path(self, task_id: str, create_root: bool = False) -> Path:
        ...
        if not path.is_relative_to(root):   # 检查路径是否在根目录下
            raise ValueError(f"task_id {task_id} 无效")
        return path
    ...
    def update_dependencies(self, 
        ...
        if not isinstance(add_blocked_by, list):
            raise ValueError("add_blocked_by 必须是关于 task_id 的依赖任务 ID 列表")

        task = self.load(task_id)
        if task.status != "pending" or task.owner is not None:
            raise ValueError("只能更新 pending 状态且无 owner 的任务的依赖")

        dependencies = list(dict.fromkeys(add_blocked_by))
        for dependency in dependencies:
            if dependency == task_id:
                raise ValueError("任务不能依赖于自己")
            if not self.exists(dependency):
                raise ValueError(f"依赖任务 {dependency} 不存在")
            # 任务 dependency 依赖于任务 task_id，不能添加到依赖列表
            if dependency not in task.blockedBy and self._depends_on(dependency, task_id):
                raise ValueError(f"依赖任务 {dependency} 依赖于任务 {task_id}，不能添加到依赖列表")
    ...
    def load(self, task_id: str) -> Task:
        ...
        task = Task(**data)
        if task.id != task_id:
            raise ValueError(f"任务 ID 与文件名不匹配，文件名: {task_id}, 任务 ID: {task.id}")
        if task.status not in ("pending", "in_progress", "completed"):
            raise ValueError(f"任务状态 {task.status} 无效")
        return task
